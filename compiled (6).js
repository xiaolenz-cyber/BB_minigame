var BattleBackground = (function () {
    ...
    BattleBackground.getWikiaLinkFromShortenedLink = function (shortenedLink) {
        var firstPart = "https://img".concat(shortenedLink.charAt(0), ".wikia.nocookie.net/bloodbrothersgame/images/");
...
var BattleDebugger = (function () {
    ...
    BattleDebugger.prototype.displayMajorEvent = function (index) {
        ...
        var id = "turn".concat(data.turn, "events");
        ...
        newEvent.innerHTML = "<a>".concat(data.description, "</a>");
    ...
    BattleDebugger.prototype.displayMinorEvent = function (majorIndex, minorIndex) {
        ...
        var id = "turn".concat(currentTurn, "events");
    ...
    BattleDebugger.prototype.displayTurn = function (turnNum) {
        ...
        newEvent.innerHTML = "Turn ".concat(turnNum);
    ...
    BattleDebugger.prototype.displayEventLogAtIndex = function (majorIndex) {
        ...
        for (var p = 1; p <= 2; p++) {
            var playerCards = field["player".concat(p, "Cards")];
            var lastPlayerCards = lastEventField["player".concat(p, "Cards")];
            for (var f = 0; f < 5; f++) {
                ...
                var infoText = {
                    ...
                    hp: "HP: ".concat(stats.hp),
                    atk: "ATK: ".concat(finalAtk),
                    def: "DEF: ".concat(finalDef),
                    wis: "WIS: ".concat(finalWis),
                    agi: "AGI: ".concat(finalAgi),
                };
                if (status_1.attackResistance !== 0)
                    infoText.physicalResist = "PW: ".concat(status_1.attackResistance);
                if (status_1.magicResistance !== 0)
                    infoText.magicalResist = "MW: ".concat(status_1.magicResistance);
                if (status_1.breathResistance !== 0)
                    infoText.breathResist = "BW: ".concat(status_1.breathResistance);
                if (status_1.willAttackAgain !== 0)
                    infoText.willAttackAgain = "Extra action: Yes";
                if (status_1.skillProbability !== 0)
                    infoText.skillProbability = "Extra prob.: ".concat(status_1.skillProbability);
                if (status_1.hpShield !== 0)
                    infoText.hpShield = "HP Shld.: ".concat(status_1.hpShield);
                if (afflict) {
                    infoText.affliction = "Affliction: ".concat(Affliction.getAfflictionAdjective(afflict.type));
                    if (afflict.type === ENUM.AfflictionType.SILENT) {
                        infoText.affliction += " (".concat(afflict.validTurnNum, " turn)");
                    }
                    else if (afflict.type === ENUM.AfflictionType.POISON) {
                        infoText.affliction += " (".concat(afflict.percent, " %)");
                    }
                    else if (afflict.type === ENUM.AfflictionType.BURN) {
                        infoText.affliction += " (".concat(afflict.damage, ")");
                    }
                    else {
                ...
                if (logger.minorEventLog[majorIndex] && logger.minorEventLog[majorIndex][0].executorId === playerCards[f].id) {
                    infoText.name = "<b>".concat(infoText.name, "</b>");
                }
                var htmlelem = document.getElementById("player".concat(p, "Fam").concat(f));
                htmlelem.innerHTML = infoText.name + "<br>" +
                    ...
                    infoText.wis + "<br>" +
                    infoText.agi +
                    (infoText.physicalResist ? "<br>".concat(infoText.physicalResist) : "") +
                    (infoText.magicalResist ? "<br>".concat(infoText.magicalResist) : "") +
                    (infoText.breathResist ? "<br>".concat(infoText.breathResist) : "") +
                    (infoText.willAttackAgain ? "<br>".concat(infoText.willAttackAgain) : "") +
                    (infoText.skillProbability ? "<br>".concat(infoText.skillProbability) : "") +
                    (infoText.hpShield ? "<br>".concat(infoText.hpShield) : "") +
                    (infoText.affliction ? "<br>".concat(infoText.affliction) : "");
                var lastEventCard = lastEventField["player".concat(p, "Cards")][f];
...
(function (ENUM) {
    ...
    (function (RarityType) {
        ...
        RarityType[RarityType["OMEGA"] = 7] = "OMEGA";
...
var BattleGraphic = (function () {
    ...
    BattleGraphic.prototype.displayFormationAndFamOnCanvas = function () {
        ...
        for (var player = 1; player <= 2; player++) {
            ...
            var groupPlayer = draw.group().attr('id', "p".concat(player, "group"));
            ...
            for (var i = 0; i < 5; i++) {
                ...
                var image = draw.image(imageLinksArray[i])
                    ...
                    .attr('id', "p".concat(player, "f").concat(i, "image"))
                ...
                var damageText = draw.text('0').font({ size: wr * 22, family: BattleGraphic.FONT })
                    ...
                    .attr('id', "p".concat(player, "f").concat(i, "damageText"))
                    .opacity(0);
                var explosion = draw.image('img/explosion.png', wr * 70, wr * 70)
                    .move(image_x_coord, image_y_coord)
                    .attr('id', "p".concat(player, "f").concat(i, "explosion"))
                    .opacity(0);
                var group = draw.group().attr('id', "p".concat(player, "f").concat(i, "group"));
                group.add(image);
    ...
    BattleGraphic.prototype.displayAllCardImages = function (majorIndex) {
        ...
        for (var p = 1; p <= 2; p++) {
            for (var f = 0; f < 5; f++) {
                var image = SVG.get("p".concat(p, "f").concat(f, "image"));
                var card = field["player".concat(p, "Cards")][f];
    ...
    BattleGraphic.prototype.displayHP = function (percent, player, index, animDuration) {
        ...
        var hpbarId = "p".concat(player, "f").concat(index, "hp");
        ...
        if (!hpbar) {
            hpbar = draw.rect(width, height)
                ...
                .attr('id', hpbarId)
                .move(xstart, ystart);
            var groupId = "p".concat(player, "f").concat(index, "group");
            var group = SVG.get(groupId);
            group.add(hpbar);
        }
        var hpGradientId = "p".concat(player, "f").concat(index, "hpGradient");
        ...
        if (!hpGradient) {
            hpGradient = draw.gradient('linear', function (stop) {
                stop.at({ offset: '100%', color: '#00ff00' }).attr('id', "p".concat(player, "f").concat(index, "hpgs1"));
                stop.at({ offset: '100%', color: 'transparent' }).attr('id', "p".concat(player, "f").concat(index, "hpgs2"));
            }).attr('id', hpGradientId);
            hpbar.fill(hpGradient);
        }
        else {
            var s1 = SVG.get("p".concat(player, "f").concat(index, "hpgs1"));
            var s2 = SVG.get("p".concat(player, "f").concat(index, "hpgs2"));
    ...
    BattleGraphic.prototype.displayDamageTextAndHP = function (playerId, famIndex, majorIndex, minorIndex) {
        ...
        var targetInfo = field["player".concat(playerId, "Cards")][famIndex];
        ...
        var damageText = SVG.get("p".concat(playerId, "f").concat(famIndex, "damageText"));
    ...
    BattleGraphic.prototype.displayAllAfflictionText = function (majorIndex) {
        ...
        for (var player = 1; player <= 2; player++) {
            for (var fam = 0; fam < 5; fam++) {
                ...
                var data = field["player".concat(player, "Cards")][fam];
                if (!data.affliction) {
                    svgAfflictTxt.hide();
                    svgAfflictTxt.text('-');
                }
    ...
    BattleGraphic.prototype.displayDeadAliveFamiliar = function (player, fam, isDead) {
        ...
        if (ENUM.Setting.IS_MOBILE) {
            image = document.getElementById("p".concat(player, "f").concat(fam, "image"));
            if (isDead) {
            ...
            image = SVG.get("p".concat(player, "f").concat(fam, "image"));
    ...
    BattleGraphic.prototype.displayProcSkill = function (executorId, skillId, option) {
        ...
        if (!option.noProcEffect) {
            ...
            SVG.get("p".concat(executor.getPlayerId(), "f").concat(executor.formationColumn, "group")).front();
        ...
        var groupSkillBg = SVG.get("p".concat(executor.getPlayerId(), "SkillBgTextGroup"));
        var svgText = SVG.get("p".concat(executor.getPlayerId(), "SkillText"));
        ...
        if (minorIndex < minorLog[majorIndex].length) {
            ...
            if (data.status.type === ENUM.StatusType.ATTACK_RESISTANCE ||
                ...
                if (data.status.isDispelled) {
                    ...
                    displayText = "Prob.".concat(data.amount < 0 ? " Down" : " Up");
                }
                ...
                var damageText = SVG.get("p".concat(target.getPlayerId(), "f").concat(target.formationColumn, "damageText"));
    ...
    BattleGraphic.prototype.displayReviveEvent = function (majorIndex, minorIndex, option) {
        ...
        executorGroup.front();
        SVG.get("p".concat(executor.getPlayerId(), "group")).front();
        if (minorIndex < minorLog[majorIndex].length) {
            var target = this.cardMan.getCardById(data.targetId);
            ...
            var damageText = SVG.get("p".concat(playerId, "f").concat(index, "damageText"));
    ...
    BattleGraphic.prototype.displayProtectEvent = function (majorIndex, minorIndex, option) {
        ...
        if (minorIndex < minorLog[majorIndex].length) {
            ...
            var explosion_1 = SVG.get("p".concat(executor.getPlayerId(), "f").concat(executor.formationColumn, "explosion"));
            executorGroup.animate({ duration: moveTime + 's' })
                ...
                if (Skill.isIndirectSkill(nextData_1.skillId)) {
                    ...
                    SVG.get("p".concat(attackerCard_1.getPlayerId(), "group")).front();
    ...
    BattleGraphic.prototype.displayBcAddProbEvent = function (majorIndex, minorIndex, option) {
        ...
        if (minorIndex < minorLog[majorIndex].length) {
            if (data.bcAddProb.isMain) {
                ...
                damageText.text("+".concat(bonusProb, "%")).center(center_x_3, center_y_3).font({ size: BattleGraphic.wr * 25 })
    ...
    BattleGraphic.prototype.displayBattleEvent = function (majorIndex, minorIndex, option) {
        ...
        executorGroup.front();
        SVG.get("p".concat(executor.getPlayerId(), "group")).front();
        var explosion = SVG.get("p".concat(target.getPlayerId(), "f").concat(target.formationColumn, "explosion"));
        if (Skill.isAoeSkill(data.skillId)) {
            ...
            for (var i = 0; i < aoeTargets.length; i++) {
                ...
                exploSet.push(SVG.get("p".concat(target.getPlayerId(), "f").concat(exploTargetCol, "explosion")));
            }
            if (noAttackAnim) {
                this.displayPostDamage(target.getPlayerId(), target.formationColumn, majorIndex, minorIndex);
                this.displayMinorEventAnimation(majorIndex, minorIndex + 1, option);
    ...
    BattleGraphic.prototype.getCardImage = function (card) {
        return SVG.get("p".concat(card.getPlayerId(), "f").concat(card.formationColumn, "image"));
    };
    BattleGraphic.prototype.getCardImageGroup = function (card) {
        return SVG.get("p".concat(card.getPlayerId(), "f").concat(card.formationColumn, "group"));
    };
    BattleGraphic.prototype.getWard = function (playerId, famIndex, type) {
        ...
        var ward = SVG.get("p".concat(playerId, "f").concat(famIndex).concat(wardTxt));
        if (!ward) {
            ward = SVG.get('mainSvg').image("img/".concat(wardFileName), BattleGraphic.wr * 70, BattleGraphic.wr * 70)
                .center(this.coordArray[playerId][famIndex][0], this.coordArray[playerId][famIndex][1])
                .attr('id', "p".concat(playerId, "f").concat(famIndex).concat(wardTxt))
                .opacity(0);
            SVG.get("p".concat(playerId, "f").concat(famIndex, "group")).add(ward);
        }
        return ward;
    };
    BattleGraphic.prototype.getAfflictionText = function (playerId, famIndex) {
        var txt = SVG.get("p".concat(playerId, "f").concat(famIndex, "afflictText"));
        if (!txt) {
            txt = SVG.get('mainSvg').text('Paralyzed').font({ size: BattleGraphic.wr * 14, family: BattleGraphic.FONT })
                ...
                .attr('id', "p".concat(playerId, "f").concat(famIndex, "afflictText"))
                .hide();
            SVG.get("p".concat(playerId, "f").concat(famIndex, "group")).add(txt);
        }
        return txt;
    };
    BattleGraphic.prototype.getProcEffect = function (playerId, famIndex, type) {
        ...
        var effect = SVG.get('mainSvg').image("img/".concat(file), BattleGraphic.wr * 150, BattleGraphic.wr * 150)
            .center(this.coordArray[playerId][famIndex][0], this.coordArray[playerId][famIndex][1])
            .attr('id', "p".concat(playerId, "f").concat(famIndex, "spellCircle"))
            .opacity(0);
        SVG.get("p".concat(playerId, "f").concat(famIndex, "group")).add(effect);
        return effect;
    };
...
var BattleLogger = (function () {
    ...
    BattleLogger.prototype.displayInfoText = function () {
        ...
        if (battle.isBloodClash) {
            if (!battle.p1RandomMode || !BattleModel.IS_MASS_SIMULATION) {
                infoDivP1.innerHTML += "<br><br>".concat(cardManager.getPlayerReserveBrigString(battle.player1));
            }
            if (!battle.p2RandomMode || !BattleModel.IS_MASS_SIMULATION) {
                infoDivP2.innerHTML += "<br><br>".concat(cardManager.getPlayerReserveBrigString(battle.player2));
            }
        }
    ...
    BattleLogger.prototype.displayWarningText = function () {
        ...
        if (needWarn) {
            ...
            var warnTxt = "<p><b>Note:</b> It seems that you using a 5v5 or 10v10 battle to determine whether ".concat(p1main[0].name, " or ").concat(p2main[0].name, " is \"stronger\". This is <b>NOT</b> a recommended way of comparing two familiars.</p>");
    ...
    BattleLogger.prototype.getRandomModeText = function (type) {
        switch (type) {
            ...
            case ENUM.RandomBrigType.A_UP:
                return " (random ".concat(ENUM.RandomBrigText[type], ")");
            default:
                return "";
        }
    };
...
var CardManager = (function () {
    ...
    CardManager.prototype.getHtmlBrigString = function (cards) {
        ...
        for (var i = 0; i < cards.length; i++) {
            var dash = (i === 0) ? "" : " - ";
            var cb = "showCardDetailDialogById(".concat(cards[i].id, ");");
...
var famDatabase = {
    ...
    41808: {
        name: "Artificial", stats: [18000, 18585, 15050, 15409, 17956],
        skills: [10198, 10199],
...
function setPreviousChoices() {
    if (localStorage.getItem("f0") && localStorage.getItem("f0") !== "null") {
        for (var i = 0; i < 10; i++) {
            document.getElementById("f".concat(i)).value = localStorage.getItem("f".concat(i));
        }
    }
    if (localStorage.getItem("f10") && localStorage.getItem("f10") !== "null") {
        for (var i = 0; i < 10; i++) {
            document.getElementById("f".concat(i + 10)).value = localStorage.getItem("f".concat(i + 10));
        }
    }
    if (localStorage.getItem("s10") && localStorage.getItem("s10") !== "null") {
        for (var i = 0; i < 3; i++) {
            document.getElementById("s1".concat(i)).value = localStorage.getItem("s1".concat(i));
        }
    }
    if (localStorage.getItem("s20") && localStorage.getItem("s20") !== "null") {
        for (var i = 0; i < 3; i++) {
            document.getElementById("s2".concat(i)).value = localStorage.getItem("s2".concat(i));
        }
    }
    ...
    for (var player = 1; player <= 2; player++) {
        for (var famIndex = 0; famIndex < 10; famIndex++) {
            for (var statIndex = 0; statIndex < arr.length; statIndex++) {
                var key = "p".concat(player, "f").concat(famIndex).concat(arr[statIndex].toLowerCase());
    ...
    for (var player = 1; player <= 2; player++) {
        for (var famIndex = 0; famIndex < 10; famIndex++) {
            for (var skillIndex = 0; skillIndex < 3; skillIndex++) {
                var key = "p".concat(player, "f").concat(famIndex, "s").concat(skillIndex);
                if (localStorage.getItem(key) && localStorage.getItem(key) !== "null") {
                    document.getElementById(key).value = localStorage.getItem(key);
                }
            }
...
function toogleDisable() {
    for (var player = 1; player <= 2; player++) {
        var isSelected = document.getElementById("r".concat(player)).checked;
        var elems = document.getElementsByClassName("p".concat(player));
...
function toogleCustomStat(player) {
    var checked = document.getElementById("p".concat(player, "customStatChbox")).checked;
    var btValue = document.getElementById("bt").value;
    var isBloodclash = btValue === "1" || btValue === "3";
    var customStatDiv = document.getElementById("p".concat(player, "customStatDivNormal"));
    ...
    customStatDiv = document.getElementById("p".concat(player, "customStatDivBloodclash"));
    ...
    var arr = ["HP", "ATK", "DEF", "WIS", "AGI"];
    for (var i = 0; i < 10; i++) {
        var slotChecked = document.getElementById("p".concat(player, "f").concat(i, "customStatChkbox")).checked;
        var slotDisabled = document.getElementById("p".concat(player, "f").concat(i, "customStatChkbox")).disabled;
        for (var j = 0; j < arr.length; j++) {
            var txtBox = document.getElementById("p".concat(player, "f").concat(i).concat(arr[j].toLowerCase()));
            txtBox.disabled = slotDisabled || !slotChecked;
        }
    }
}
function toogleCustomSkill(player) {
    var checked = document.getElementById("p".concat(player, "customSkillChbox")).checked;
    var btValue = document.getElementById("bt").value;
    var isBloodclash = btValue === "1" || btValue === "3";
    var customSkillDiv = document.getElementById("p".concat(player, "customSkillDivNormal"));
    var inputs = customSkillDiv.getElementsByTagName("input");
    for (var i = 0; i < inputs.length; i++) {
        inputs[i].disabled = !checked;
    }
    customSkillDiv.style.display = checked ? "block" : "none";
    customSkillDiv = document.getElementById("p".concat(player, "customSkillDivBloodclash"));
    ...
    for (var i = 0; i < 10; i++) {
        var slotChecked = document.getElementById("p".concat(player, "f").concat(i, "customSkillChkbox")).checked;
        var slotDisabled = document.getElementById("p".concat(player, "f").concat(i, "customSkillChkbox")).disabled;
        for (var j = 0; j < 3; j++) {
            var select = document.getElementById("p".concat(player, "f").concat(i, "s").concat(j));
...
function addCustomsStatDiv() {
    ...
    for (var divIndex = 0; divIndex < divArr.length; divIndex++) {
        for (var player = 1; player <= 2; player++) {
            var customStatDiv = document.getElementById("p".concat(player, "customStatDiv").concat(divArr[divIndex]));
            for (var i = 0; i < 5; i++) {
                var famIndex = i + 5 * divIndex;
                customStatDiv.appendChild(document.createElement('br'));
                customStatDiv.appendChild(document.createTextNode("Familiar ".concat(famIndex + 1)));
                var input = document.createElement("input");
                input.type = "checkbox";
                input.id = "p".concat(player, "f").concat(famIndex, "customStatChkbox");
                input.onchange = (function (p) { return function () { return toogleCustomStat(p); }; })(player);
                customStatDiv.appendChild(input);
                for (var statIndex = 0; statIndex < arr.length; statIndex++) {
                    customStatDiv.appendChild(document.createTextNode(" ".concat(arr[statIndex], " ")));
                    input = document.createElement("input");
                    input.type = "text";
                    input.name = "p".concat(player, "f").concat(famIndex).concat(arr[statIndex].toLowerCase());
                    input.id = "p".concat(player, "f").concat(famIndex).concat(arr[statIndex].toLowerCase());
                    input.size = 6;
                    input.maxLength = 6;
                    customStatDiv.appendChild(input);
                }
...
function addCustomsSkillDiv() {
    ...
    for (var divIndex = 0; divIndex < divArr.length; divIndex++) {
        for (var player = 1; player <= 2; player++) {
            var customSkillDiv = document.getElementById("p".concat(player, "customSkillDiv").concat(divArr[divIndex]));
            for (var i = 0; i < 5; i++) {
                var famIndex = i + 5 * divIndex;
                customSkillDiv.appendChild(document.createElement('br'));
                customSkillDiv.appendChild(document.createTextNode("Familiar ".concat(famIndex + 1)));
                var input = document.createElement("input");
                input.type = "checkbox";
                input.id = "p".concat(player, "f").concat(famIndex, "customSkillChkbox");
                input.onchange = (function (p) { return function () { return toogleCustomSkill(p); }; })(player);
                customSkillDiv.appendChild(input);
                for (var skillIndex = 0; skillIndex < 3; skillIndex++) {
                    customSkillDiv.appendChild(document.createTextNode(" Skill ".concat(skillIndex, " ")));
                    var select = document.createElement("select");
                    select.name = "p".concat(player, "f").concat(famIndex, "s").concat(skillIndex);
                    select.id = "p".concat(player, "f").concat(famIndex, "s").concat(skillIndex);
...
function getBattleDataOption() {
    ...
    for (var player = 1; player <= 2; player++) {
        for (var famIndex = 0; famIndex < 10; famIndex++) {
            for (var statIndex = 0; statIndex < arr.length; statIndex++) {
                ...
                if (stat) {
                    if (!data["p".concat(player, "_customStats")][famIndex]) {
                        data["p".concat(player, "_customStats")][famIndex] = {};
                    }
                    data["p".concat(player, "_customStats")][famIndex][arr[statIndex].toLowerCase()] = +stat;
    ...
    for (var player = 1; player <= 2; player++) {
        for (var famIndex = 0; famIndex < 10; famIndex++) {
            for (var skillIndex = 0; skillIndex < 3; skillIndex++) {
                ...
                if (customSkillId) {
                    if (!data["p".concat(player, "_customSkills")][famIndex]) {
                        data["p".concat(player, "_customSkills")][famIndex] = [];
                    }
                    data["p".concat(player, "_customSkills")][famIndex][skillIndex] = +customSkillId;
    ...
    for (var i = 0; i < 10; i++) {
        var f1id = getURLParameter("f".concat(i));
        var f2id = getURLParameter("f".concat(i + 10));
        data.p1_cardIds.push(f1id);
        data.p2_cardIds.push(f2id);
        if (!option.p1RandomMode)
            localStorage.setItem("f".concat(i), f1id);
        if (!option.p2RandomMode)
            localStorage.setItem("f".concat(i + 10), f2id);
    }
    for (var i = 0; i < 3; i++) {
        var w1s = getURLParameter("s1".concat(i));
        var w2s = getURLParameter("s2".concat(i));
        data.p1_warlordSkillIds.push(w1s);
        data.p2_warlordSkillIds.push(w2s);
        if (!option.p1RandomMode)
            localStorage.setItem("s1".concat(i), w1s);
        if (!option.p2RandomMode)
            localStorage.setItem("s2".concat(i), w2s);
    }
    return [data, option];
}
...
function prepareField() {
    ...
    img.onload = function () {
        svgWrapper.style.backgroundImage = "url('".concat(rndBgLink, "')");
    };
    img.onerror = function () {
        console.log("Background not found: ".concat(rndBgLink));
...
function onSimulationResultObtained(finalData, startTime, endTime) {
    ...
    simResultDiv.innerHTML += ("Player 2 won: ".concat(finalData.p2WinCount, "<br> Player 1 won: ").concat(finalData.p1WinCount, "<br><br> Time: ").concat(((endTime - startTime) / 1000).toFixed(2), "s<br><a href=setting.html>Go back to main page </a>"));
...
var TurnOrderChangeSkillLogic = (function (_super) {
    ...
    TurnOrderChangeSkillLogic.prototype.execute = function (data) {
        ...
        this.logger.addMinorEvent({
            ...
            description: "Turn order is now based on ".concat(ENUM.BattleTurnOrderType[data.skill.skillFuncArg1], " for ").concat(data.skill.skillFuncArg2, " turn(s)."),
            skillId: data.skill.id,
...
function getScaledFamiliarWikiaImageLink(link, fullName, newWidth) {
    var firstPart = "https://img".concat(link.charAt(0), ".wikia.nocookie.net/bloodbrothersgame/images/thumb/").concat(link.charAt(1), "/").concat(link.substring(1), "/");
...
var BattleModel = (function () {
    ...
    BattleModel.prototype.getWouldBeDamage = function (data) {
        ...
        switch (skill.skillCalcType) {
            ...
            case ENUM.SkillCalcType.REFLECT:
                ...
                break;
            case (ENUM.SkillCalcType.ATK_WIS):
                baseDamage = getDamageCalculatedByATK(attacker, target, ignorePosition) + getDamageCalculatedByWIS(attacker, target);
                break;
            case (ENUM.SkillCalcType.ATK_AGI):
                baseDamage = getDamageCalculatedByATK(attacker, target, ignorePosition) + getDamageCalculatedByAGI(attacker, target, ignorePosition);
                break;
            case (ENUM.SkillCalcType.WIS_AGI):
                baseDamage = getDamageCalculatedByWIS(attacker, target) + getDamageCalculatedByAGI(attacker, target, ignorePosition);
                break;
            default:
    ...
    BattleModel.prototype.processEndTurn = function () {
        ...
        if (this.logger.currentTurn >= BattleModel.MAX_TURN_NUM) {
            ...
            this.isFinished = true;
            this.logger.addMajorEvent({
                description: "Decision win for ".concat(this.playerWon.name)
            });
            this.logger.addMinorEvent({
                type: ENUM.MinorEventType.BATTLE_DESCRIPTION,
            ...
            for (var i = 0; i < allCards.length; i++) {
                ...
                if (tmpCard && !tmpCard.isDead) {
                    ...
                    this.logger.addMinorEvent({
                        ...
                        description: "".concat(tmpCard.name, " gets ").concat(bonusProb, "% increase in skill prob."),
                        bcAddProb: {
                            targetId: tmpCard.id,
